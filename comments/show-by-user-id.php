<?php 
    ...
    $limit = isset($_GET['_limit']) && $_GET['_limit'] != '' ? (int)$_GET['_limit'] : 1;
    $page = isset($_GET['_page']) && $_GET['_page'] != '' ? (int)$_GET['_page'] : 1;

    $sql = "SELECT * FROM comments WHERE user_id = '$user_id' AND cmt_parent_id = 0";
    $rl = mysqli_query($conn, $sql);
    ...
    $sql = "SELECT * FROM comments INNER JOIN user ON comments.user_id = user.user_id WHERE comments.user_id = '$user_id' AND cmt_parent_id = 0 ORDER BY cmt_id DESC LIMIT $start, $limit";
    $rl = mysqli_query($conn, $sql);
    ...
    while ($row = mysqli_fetch_assoc($rl)) {
        ...
        $sql2 = "SELECT * FROM comments INNER JOIN user ON comments.user_id = user.user_id WHERE cmt_parent_id = '$cmt_id' ORDER BY cmt_id ASC";
