<?php 
    ...
    $search_id = $_GET['_id'];
    $search_status_id = $_GET['_status_id'];
    $limit = isset($_GET['_limit']) && $_GET['_limit'] != '' ? (int)$_GET['_limit'] : 10;
    $page = isset($_GET['_page']) && $_GET['_page'] != '' ? (int)$_GET['_page'] : 1;

    $params = '';
    if ($search_status_id) {
        $params .= isset($params) && $params == '' ? " WHERE orders.order_status_id = '$search_status_id' " : " AND orders.order_status_id = '$search_status_id' ";
    }
    if ($search_id) {
        $params .= isset($params) && $params == '' ? " WHERE orders.order_id = '$search_id' " : " AND orders.order_id = '$search_id' ";
    }
    ...
    $sql2 = "SELECT * FROM orders 
            ...
            ".$params." ORDER BY order_id DESC LIMIT $start,".$res['limit'];
